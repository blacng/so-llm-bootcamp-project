...
class ChatbotUI:
    ...
    @staticmethod
    def setup_page(
        title: str, icon: str, layout: str = "wide", sidebar_state: str = "collapsed"
    ) -> None:
    ...
    def render_chat_message(role: str, content: str, avatar_url: str = None) -> None:
        ...
        if avatar_url is None:
            avatar_url = (
                ChatbotUI.get_user_avatar()
                if role == "user"
                else ChatbotUI.get_bot_avatar()
            )
    ...
    @staticmethod
    def render_processing_message(
        message: str = "Processing...", avatar_url: str = None
    ):
    ...
    def display_chat_messages(messages: list) -> bool:
        ...
        for message in messages:
            ChatbotUI.render_chat_message(message["role"], message["content"])
        return True
...
class APIKeyUI:
    ...
    def render_api_key_form(
        title: str = "🔑 Enter API Key", inputs: list = None
    ) -> dict:
            ...
            if inputs:
                for input_config in inputs:
                    input_values[input_config["key"]] = st.text_input(
                        input_config["label"],
                        type="password"
                        if input_config.get("password", True)
                        else "text",
