...
def check_python_version():
    ...
    if version.major >= 3 and version.minor >= 8:
        print_status(
            f"Python {version.major}.{version.minor}.{version.micro} - OK", "SUCCESS"
        )
        return True
    else:
        print_status(
...
def test_basic_chatbot_import():
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location(
            "basic_chatbot", "pages/1_Basic_Chatbot.py"
        )
...
def main():
    ...
    print_status(
        f"Passed: {passed}/{total} checks", "SUCCESS" if passed == total else "WARNING"
    )
    ...
    if passed < total:
        ...
        print_status(
            "All checks passed! Chatbot should be working properly.", "SUCCESS"
        )
